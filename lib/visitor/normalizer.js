...
Normalizer.prototype.checkForPrefixedGroups = function (selector) {
  var prefix = [],
    result = null,
    stackElement = null,
    fullSelector = '';
  ...
  for (var i = 0; i < this.stack.length; i++) {
    stackElement = this.stack[i] && this.stack[i][0];
    if (!stackElement) continue;

    prefix.push(stackElement.val);
    fullSelector = prefix.join(' ') + ' ' + selector;
    result = this.map[fullSelector];
...
Normalizer.prototype.extend = function(group, selectors){
  ...
  group.extends.forEach(function(extend){
    var groups = self.checkForPrefixedGroups(extend.selector) && map[extend.selector]; // backward compatibility may be broken
    if (!groups) {
